...
class Wide_PayFiscalModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        $sql = "SELECT * FROM `"._DB_PREFIX_."wide_pay` WHERE id_cliente = '".(int)$cliente->id."'";
        ...
        if(strlen($cpf_cnpj)==11 || strlen($cpf_cnpj)==14){
           ...
           Tools::redirect(Context::getContext()->link->getModuleLink('wide_pay', $tipo, array(), true));
        }else{
        $campo_fiscal = Configuration::get('WIDE_PAY_CPF_CNPJ');
        if(!empty($campo_fiscal)){
			...
			if(isset($array_cobranca[$partes[1]])){
                ...
                if(strlen($cpfCnpjComprador)==11 || strlen($cpfCnpjComprador)==14){
                    ...
                    if(!isset($row['fiscal'])){
                        $query = "INSERT INTO `"._DB_PREFIX_."wide_pay` (`id_cliente`, `fiscal`) VALUES ('".(int)$cliente->id."', '".$cpf_cnpj."');";
                        Db::getInstance()->execute($query);
                    }else{
                        $query = "UPDATE `"._DB_PREFIX_."wide_pay` SET `fiscal` = '".$cpf_cnpj."' WHERE `id_cliente` = '".(int)$cliente->id."'";
                        Db::getInstance()->execute($query);
                    }
                    //redireciona ao pagamento
                    Tools::redirect(Context::getContext()->link->getModuleLink('wide_pay', $tipo, array(), true));
        ...
        return $this->setTemplate('module:wide_pay/views/templates/front/fiscal.tpl');
    }
}
