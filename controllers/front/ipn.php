...
class Wide_PayIpnModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
		...
		if(!empty($transacao) && !empty($id_notificacao) && $_POST['apiKey']==trim(Configuration::get('WIDE_PAY_ID'))){
			$json = array();
			$json['token'] = trim(Configuration::get('WIDE_PAY_TOKEN'));
			$json['apiKey'] = trim(Configuration::get('WIDE_PAY_ID'));
			$json['transaction_id'] = trim($transacao);
			$json['notification_id'] = trim($id_notificacao);
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, 'https://api.widepay.com/transaction/notification/');
			...
			if(isset($retorno['status_request']['result']) && $retorno['status_request']['result']=='success'){
				...
				$pedido = Db::getInstance()->getRow("SELECT * FROM `"._DB_PREFIX_."orders` WHERE module = 'wide_pay' AND id_cart = '".(int)$retorno['status_request']['order_id']."' AND current_state = '".Configuration::get('WIDE_PAY_WATING_PAYMENT')."'");
				...
				if($order){
					if($retorno['status_request']['status']=='paid'){
						...
						$history = new OrderHistory();
						$history->id_order = (int)$order->id;
						$history->changeIdOrderState(Configuration::get('WIDE_PAY_CANCELLED'), $order);
						$history->addWithemail(true, null);
					}
				}
			}
